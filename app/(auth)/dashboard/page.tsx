...
<link rel="stylesheet"  href="https://templates.smadmail.com/css/iframe.css"/>
  ...
  const [isOpenMapShowCodeDialog, setIsOpenMapShowCodeDialog] = useState<boolean>(false)
  const [isOpenMapCreatedModal, setIsOpenMapCreatedModal] = useState<boolean>(false) // New state for the map creation modal
...
<AlertDialog open={isOpenMapCreatedModal}>
  <AlertDialogContent className="bg-neutral-900">
    <AlertDialogHeader>
      ...
      <AlertDialogDescription>
        <div className="flex flex-col justify-start items-start py-3 w-full">
          <p className="text-sm text-neutral-400">Your map has been created. Copy the code below to embed it:</p>
          <textarea
            readOnly

...
const ProjectTabItem = ({
  ...
  const [isProjectNameUpdate, setIsProjectNameUpdate] =
  ...
  const [isCodeCopyCodeScriptEmail, setIsCodeCopyCodeScriptEmail] =
    useState(false);
  ...
  const mutationUpdateProjectName = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["getAllProjectsOneUser", user?.id],
      });
  ...
  const codeScriptEmail = generateCodeScript(
    withName,
    privateKey,
    project?.id || "",
  );
  ...
  return (
    <>
      ...
      <AlertDialog open={isOpenDelete}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <ButtonValidation
              title={"confirm"}
              isLoading={mutationDeleteProject.isPending}
              typeButton="button"
              type="negative"
      ...
      <button
        ...
        onClick={onClick}
        className={cn(
          "py-2 flex px-3 text-sm rounded-md w-full justify-between items-center gap-x-3",
          className,
      ...
      >
        <div className="flex gap-2 justify-start items-center w-full">
          <svg
            ...
            strokeLinecap="round"
            strokeLinejoin="round"
            className="lucide lucide-table2 grid-area-1-1"
          >
          ...
          <div className="w-6 h-6 flex justify-start items-start">
            {mutationUpdateProjectName?.isPending && (
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="bg-neutral-900" align="end">
            ...
            <DropdownMenuItem
              className="cursor-pointer"
              onClick={handleOpenScriptCodeEmail}
            >
              Show code integration Email
            </DropdownMenuItem>
            <DropdownMenuSeparator
              className={`${withMap ? "block" : "hidden"}`}
            />
            {withMap && (
              <DropdownMenuItem
                className="cursor-pointer"
                onClick={handleOpenScriptCodeMap}
              >
                Show code integration Map
              </DropdownMenuItem>
            )}
            <DropdownMenuSeparator />
            <DropdownMenuItem
              className="cursor-pointer"
              onClick={() => {
                setIsProjectNameUpdate(true);
                setIdProject(project.id);
                setNewNameProject(project.name);
              }}
            >
              Update
            </DropdownMenuItem>

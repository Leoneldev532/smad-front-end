import { API_URL, CONTENT_TYPE } from "@/lib/constants";
...
import { saveEmailToExternalAPI } from "@/hook/query";
const FormRegisterEmail3 = () => {
  ...
  const useEmailForm = (
    ...
    const handleSubmit = async () => {
      ...
      setLoading(true);
      try {
        await saveEmailToExternalAPI(data.email, project_id, private_key);
        setSuccess(true);
      } catch (error) {
  ...
  const { data, setData, errors, handleSubmit, loading, success } =
    useEmailForm(project_id, private_key);

  // Constantes pour les classes CSS
  const formClasses =
    "flex bg-neutral-700 border border-neutral-700/40 py-0.5 px-0.5 overflow-hidden rounded-full justify-between";
  const buttonClasses =
    " w-1/5 rounded-full py-1  group  flex justify-center items-center text-black font-bold hover:bg-neutral-900 hover:text-white transition-all ease duration-300  text-sm bg-yellow-500";
  const inputClasses =
    "px-2.5 appearance-none   text-neutral-400 font-medium py-2.5 outline-none text-sm w-full bg-neutral-700 text-capitalize";
  ...
  return (
    <section className="flex flex-col py-10">
      ...
      {errors.email && (
        <p className="text-red-500 mt-2 w-full text-center">{errors.email}</p>
      )}
    </section>
  );
};
export default FormRegisterEmail3;
